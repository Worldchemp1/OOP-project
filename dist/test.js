...
export function testProductManager() {
    ...
    products.forEach((product) => {
        try {
            ...
            console.log(`✅ Created: ${created.name} (ID: ${created.id}) - $${created.price}`);
        }
        catch (error) {
    ...
    allProducts.forEach((product) => {
        console.log(`   • ${product.name} (ID: ${product.id}) - $${product.price} - ${product.category}`);
    });
    // Get product by ID
    console.log("\n🔍 Get Product by ID:");
    const productById = productManager.getProductById(2);
    if (productById) {
        console.log(`✅ Found: ${productById.name} - $${productById.price} - ${productById.category}`);
...
if (typeof window !== "undefined") {
    window.testProductManager = testProductManager;
}
else {
